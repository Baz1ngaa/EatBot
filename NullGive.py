...
for i in range(len(allLogins)):
    print(allLogins[i][i])
    sql.execute(f'UPDATE profileTel SET BFallowed = 0 WHERE login = "{allLogins[i][i]}"')
    db.commit()
    sql.execute(f'UPDATE profileTel SET LNallowed = 0 WHERE login = "{allLogins[i][i]}"')
    db.commit()
    sql.execute(f'UPDATE profileTel SET EVallowed = 0 WHERE login = "{allLogins[i][i]}"')
    db.commit()
    print("ready")
